...
<head>
    ...
    <title>Chỉnh sửa PDF bài giảng</title>
        ...
        <div class="row">
            ...
            <div class="col-md-10">
              <h1>Chỉnh sửa PDF bài giảng</h1> 
                <?php
                    ...
                    $bg_ma = $_GET['bg_ma'];
                    //Chuẩn bị câu lệnh
                    $sqlSelectBAIGIANG_MuonSua = "
                    SELECT *
                    FROM baigiang
                    WHERE bg_ma = $bg_ma;
                    ";
                    //Thực thi câu lệnh
                    $resultBaigiang_Muonsua = mysqli_query($conn, $sqlSelectBAIGIANG_MuonSua);
                    //Phan tách dữ liệu thành array PHP
                    $dataBAIGIANG_MuonSua = mysqli_fetch_array($resultBaigiang_Muonsua, MYSQLI_ASSOC);
            ...
            <form name="frmCreate" id="frmCreate" method="post"  action="" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <select name="kh_makhoahoc" id="kh_makhoahoc" class="form-control">
                        <?php foreach($dataKhoaHoc as $kh): ?>
                            <?php if( $dataBAIGIANG_MuonSua['kh_makhoahoc'] == $kh['kh_makhoahoc']): ?>
                ...
                <div class="form-group">
                    Bài (*):
                    <br/>
                    <input type="text" name="bg_bai" id="bg_bai"
                        value="<?= $dataBAIGIANG_MuonSua['bg_bai']?>"
                    class="form-control"/>
                    <br />
                    ...
                    <label for="">PDF bài giảng Khóa học</label>
                    <br />
                        <iframe src="/learnforever.xyz/assets/uploadsbaigiang/<?= $dataBAIGIANG_MuonSua['bg_tentaptin']?>" width="70%" height="400px">  </iframe>
                    <br /><br />
                        ...
                        if( isset($_FILES['bg_tentaptin'] )) {
                            ...
                            if ($_FILES['bg_tentaptin']['error'] > 0) {
                                echo 'File Upload Bị Lỗi'; die;
                                    ...
                                    $filePath = __DIR__ . '/../../assets/uploadsbaigiang/' .$dataBAIGIANG_MuonSua['bg_tentaptin'];
                                    unlink($filePath);
                                // Để tránh trường hợp có 2 người dùng cùng lúc upload tập tin trùng tên nhau
                                $baigiang_tentaptin = $_FILES['bg_tentaptin']['name'];
                                $tentaptin = date('YmdHis') . '_' . $baigiang_tentaptin; //20200530154922_hoahong.jpg
